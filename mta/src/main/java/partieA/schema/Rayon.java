...
/**
 * Classe Rayon représentant un rayon d'un seul produit Objet partagé entre les
 * threads clients et chef de rayon
 * 
...
public class Rayon {
	...
	/**
	 * Méthode atomique permettant au thread client de prendre un article dans
	 * le rayon
	 * 
	 * @param client
	 *            Le client voulant prendre un article dans le rayon
	 ...
	 * 
	 * @param stockChef
	 *            Stock personnel du chef de rayon correspondant au bon produit
	 * @return
	 */
	public synchronized int remplirStock(int stockChef) {
		// Tant que le stock n'est pas plein ou que le stock perso du chef n'est
		// pas vide
		// Il rempli le rayon
		while (stockArticle < Supermarche.RAYON_STOCK_MAX) {
			if(stockChef>0){
				stockArticle++;
				stockChef--;
